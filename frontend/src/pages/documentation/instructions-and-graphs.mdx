...
Raw instructions is a low level representation of ASM bytecode grouped into `JcRawInst` and `JcRawExpr`. There are 13 types of raw instructions:

1. JcRawAssignInst
2. JcRawThrowInst
3. JcRawCatchInst
4. JcRawEnterMonitorInst
5. JcRawLabelInst
6. JcRawLineNumberInst
7. JcRawExitMonitorInst
8. JcRawCallInst
9. JcRawReturnInst
10. JcRawBranchingInst
11. JcRawIfInst
12. JcRawSwitchInst
13. JcRawGotoInst

Raw instructions is accessible by `method.rawInstList` property (or `method.getRawInstList()` in Java). Raw instructions
don't hold any `classes`, `fields` or `methods` references to particular calls.

Raw instructions can be transformed by implementation of `JcInstExtFeature#transformRawInstList` [see here](./api-ref/features#JcInstExtFeature).
This transformation will affect all upper levels of instructions representation (instruction and control flow graph).

## Instructions

`JcInst` represents `JcRawInst` with all resolved references, ie if there is method call instruction then method, value and
arguments will be available in API.

# Control flow graph API

A control flow graph for a method is represented as a `JcGraph` object. To create `JcGraph` for a given method, you can invoke the `graph` function of a three-address instruction list:
<JavaKotlinCodeBlock kotlinCode={`
    val instructionList = method.instructionList
    val graph = method.flowGraph()
`} javaCode={`
    var instructionList = method.getInstructionList()
...
An intermediate representation of `JcGraph` uses the resolved type information (i.e., the `JcType` hierarchy) and
classpath information, therefore it requires a classpath instance. Similar to `JcRawInstList`, `JcGraph` stores a
list of method instructions. However, it also tries to resolve all the execution paths in the method. `JcGraph`
operates with the `JcInst` class hierarchy (which is often similar to `JcRawInst`) and provides the following API:
...
exception thrown by `inst`. Represents an exception control flow of a method.
...
* `entry: JcBasicBlock` &mdash; An entry of a method. There can be only one entry.
...
CFG API operates on `JcInst` instructions. `JcInst` is similar to `JcRawInst` with
...
replaced with a simple `String.concat` method call.
...
<JavaKotlinCodeBlock
    javaCode={`
    var a = jcInst.accept(new MyInstVisitor())
    var b = jcExpr.accept(new MyExprVisitor())
    `}
    kotlinCode={`
