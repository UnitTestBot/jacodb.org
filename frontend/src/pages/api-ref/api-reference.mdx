...
JcSettings is used for creating instance of `JacoDB` instance.

#### useJavaRuntime(file)

Use custom (not current process) java runtime for bytecode analysis.

There are two shortcuts `JcSettings#useProcessJavaRuntime` for using current process runtime (default option) and `JcSettings#useJavaHomeRuntime` for using Java runtime from JAVA_HOME environment variable
...
Which features jacodb will use
...
`JcDatabase` instance created based on settings. If instance is not needed yet then `close` method should be called.
...
Creates classpath instances. Not processed bytecode locations will be processed before `JcClasspath` instance will be created.
All JVM runtime locations are included by default.

| parameter  | type            | description                                                      |
|------------|-----------------|------------------------------------------------------------------|
| dirOrJars  | List of `File`  | List of files with bytecode to be loaded included into classpath |

#### load(dirOrJars)

Load specified bytecode locations into database
...
Declares scope of `JcByteCodeLocation` for resolving class references.
...
- method parameters should not reference this class in any way (i.e we are looking for class A, then A, ListA, B extends A are prohibited)
